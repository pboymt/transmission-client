...
class Transmission extends events_1.EventEmitter {
     ...
     */
    async addFile(filePath, options = null) {
        var self = this;
     ...
     */
    async addBase64(fileb64, options = null) {
        return await this.addTorrentDataSrc({
            metainfo: fileb64
        }, options);
    }
     ...
     */
    async addHash(HASH, options = null) {
     ...
     */
    async addURL(URL, options = null) {
        return await this.addTorrentDataSrc({
            filename: URL
        }, options);
    }
    async addTorrentDataSrc(args, options) {
        if (typeof options === 'object') {
            // let keys = Object.keys(options);
            // for (let key of keys) {
            //     args[key] = options[key];
     ...
     */
    async remove(ids, deleteIt = false) {
        ids = Array.isArray(ids) ? ids : [ids];
        let options = {
            arguments: {
                ids: ids,
     ...
     * Moving a Torrent
     */
    async move(ids, newLocation, moveNow = true) {
        ids = Array.isArray(ids) ? ids : [ids];
     ...
     * Rename a Torrent.
     */
    async rename(id, oldName, newName) {
        let ids = [id];
        let options = {
            arguments: {
                ids: ids,
     ...
     * waitForState
     */
    async waitForState(id, targetState) {
        let self = this;
